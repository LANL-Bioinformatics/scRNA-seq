    ...
    for term in args.enrich_terms:
        ...
        try:
            ax = gp.plot.dotplot(pre_res.res2d,
                        ...
                        show_ring=False, 
                        ofname=os.path.join(out_dir, prefix+"_"+term+"_dotplot_enrichment.png"))
        except ValueError as e: #moves on if nothing meets the cutoff
            print(e)
            print("Skipping dotplot")

        terms = pre_res.res2d.Term
        axs = pre_res.plot(terms=terms[1], ofname=os.path.join(out_dir, prefix+"_"+term+"_top_term_enrichment.png"))

        axs = pre_res.plot(terms=terms[1:args.n_top_term_enrich_plot+1],
                        #legend_kws={'loc': (1.2, 0)}, # set the legend loc
                        show_ranking=True, # whether to show the second yaxis
                        figsize=(3,4),
                        ofname=os.path.join(out_dir, prefix+"_"+term+"_top_"+str(args.n_top_term_enrich_plot)+"_term_enrichment.png")
                        )
        ...
        pre_res.res2d.to_csv(os.path.join(out_dir, prefix+"_"+term+"_enrichment_scores.csv"))
